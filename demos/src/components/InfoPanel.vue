    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <div class="info-panel-text" v-else-if="currentView === 'squeezenet-v1.1'">
          <p>In contrast to ResNet-50 and Inception-V3, the size of the weights file for SqueezeNet is tiny at 1.3 MB (using 8-bit quantization). We use the architecture and pretrained weights from <a target="_blank" rel="noopener noreferrer" href="https://github.com/rcmalli/keras-squeezenet">rcmalli/keras-squeezenet</a> (original at <a target="_blank" rel="noopener noreferrer" href="https://github.com/DeepScale/SqueezeNet">DeepScale/SqueezeNet</a>). See Jupyter notebook <a target="_blank" rel="noopener noreferrer" href="https://github.com/dhyeyinf/models-js/blob/master/notebooks/demos/squeezenet_v1.1.ipynb">here</a>.</p><p>Enter any valid image URL as input to the network. You can also select from a list of prepopulated image URLs. The endpoint must have CORS enabled, to enable us to extract the numeric data from the canvas element, so not all URLs will work. Imgur and <a target="_blank" rel="noopener noreferrer" href="https://www.flickr.com/search/?text=&license=2%2C3%2C4%2C5%2C6%2C9&sort=interestingness-desc">Flickr creative commons</a> all work, and are good places to start. After running the network, the top-5 classes are displayed. Keep in mind also we are limited to the <a target="_blank" rel="noopener noreferrer" href="https://github.com/dhyeyinf/models-js/blob/master/demos/src/utils/imagenet.js">1,000 classes of ImageNet</a>.</p><p>All computation performed entirely in your browser. Toggling GPU on should offer significant speedups compared to CPU.</p>
        </div>
        <div class="info-panel-text" v-else-if="currentView === 'imdb-bidirectional-lstm'">
          <p>This demo is modified from the Keras <a target="_blank" rel="noopener noreferrer" href="https://github.com/keras-team/keras/blob/master/examples/imdb_bidirectional_lstm.py">example</a> demonstrating the Bidirectional wrapper class around an LSTM layer. Click on "load sample text" to populate the textbox with a sample IMDB movie review (preprocessed) from the test set (not used during training). You can also enter your own text into the textbox, but keep in mind the model was trained on IMDB movie reviews only (see the corresponding <a target="_blank" rel="noopener noreferrer" href="https://github.com/dhyeyinf/models-js/blob/master/notebooks/demos/imdb_bidirectional_lstm.ipynb">Jupyter notebook</a>).</p><p>The result is a number from 0 (negative) to 1 (positive). We visualize the contributions from each word by running the forward+backward concatenated hidden state corresponding to each word through the final Dense layer.</p>
        </div>
        <div class="info-panel-text" v-else-if="currentView === 'image-super-resolution'">
          <p>This demonstrates several CNN models for super-resolution. They are adapted from GitHub repo <a target="_blank" rel="noopener noreferrer" href="https://github.com/titu1994/Image-Super-Resolution">titu1994/Image-Super-Resolution</a>. See the corresponding <a target="_blank" rel="noopener noreferrer" href="https://github.com/dhyeyinf/models-js/blob/master/notebooks/demos/image_super_resolution.ipynb">Jupyter notebook</a> preparing the models for Keras.js.</p><p>Distill ResNet SR and ResNet SR will produce sharper images while SR CNN, Expanded SR CNN, and Deep Denoising Autoencoder SR CNN will produce more subtle results. The best model may vary depending on the characteristics of the input image.</p><p>All computation performed entirely in your browser. Images are not uploaded to any servers.</p>
        </div>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
